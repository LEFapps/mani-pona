			...
			create: ({ peerId, amount }) =>
				new Promise((resolve, reject) => {
					if (this.fail === 'unknown_id') {
						reject(new ManiError(`Onbekend peerId ${peerId}`));
					} else if (!_.isNumber(amount)) {
						reject(new ManiError('Niet ondersteund: geen bedrag opgegeven'));
					} else if (this.fail === 'timeout') {
						reject(new ManiError('Transactie is verlopen'));
					} else {
						resolve({
							message: 'Transactie geslaagd',
							amount: MANI(amount)
						});
					}
				}),
			listen: () =>
				new Promise((resolve, reject) => {
					if (this.fail === 'timeout') {
						...
						resolve({
							message: 'Bevestig de transactie',
							amount: MANI(7.5)
						});
					}
				})
